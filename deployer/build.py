...
def build_package_for_inbound(name: str, ref: str, skip_check_archive_exist=False) -> bool:
    ...
    try:
        ...
        # TODO: maybe add some with parent folder for build?
        build_dir = "{}_{}".format(settings.BUILD_DIR, ref)
        os.mkdir(build_dir)
...
def get_changes_from_git_diff(mock=False):
    if mock:
        ...
        target_branch = os.environ[settings.CI_MERGE_REQUEST_TARGET_BRANCH_NAME]
        source_branch = os.environ[settings.CI_MERGE_REQUEST_SOURCE_BRANCH_NAME]
        return GitOperation.diff_branches(target_branch, source_branch)
...
def get_packages_from_changes(changes) -> set:
    ...
    """
    return set(filter(is_package_to_exclude,
